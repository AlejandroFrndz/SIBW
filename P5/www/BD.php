...
class BD{
    ...
    public function publicarEvento($idEv,$vis){
        $stmt = $this->$con->prepare("UPDATE Evento SET publicado = ? WHERE DB_idEv = ?");
        $stmt->bind_param("ii", $vis, $idEv);
        $stmt->execute();
    }
    ...
    public function getImagenesPortada(){
        $res = $this->$con->query("SELECT Imagen.id,Imagen.href,Imagen.src,Imagen.alt,Evento.publicado FROM Imagen JOIN Evento ON DB_idIm = imagenPortada ORDER BY DB_idEv ASC;");
        $imagenes = $res->fetch_all(MYSQLI_ASSOC);
        return $imagenes;
    }
